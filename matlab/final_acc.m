...
data = readmatrix('D:\科研\学位论文\图\逆瓶颈优化\准确率\3-96-4.csv');
epochs = data(:,2);
ConvNeXt_T = smooth(data(:,3),0.6);

data = readmatrix('D:\科研\学位论文\图\逆瓶颈优化\准确率\1-24-4.csv');
ConvNeXt_24 = smooth(data(:,3),0.6);

data = readmatrix('D:\科研\学位论文\图\逆瓶颈优化\准确率\1-24-2.csv');
...
set(gca,'fontsize',12)
grid minor;
title('逆瓶颈结构优化模型准确率实验','fontsize',14);
xlabel('训练轮次','fontsize',14);
ylabel('准确率','fontsize',14);
hold off;
legend([p1, p2, p3],{'ConvNeXt-T','逆瓶颈24通道','半逆瓶颈24通道'},'fontsize',12,'Location','southeast')
